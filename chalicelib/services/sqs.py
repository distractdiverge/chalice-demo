...
class SQSClient:
    ...
    def _to_message_attributes(
        self, record: PPPSummaryRecord
    ...
    def send_message(self, ppp_summary_record: PPPSummaryRecord) -> bool:
        ...
        try:
            self._client.send_message(
                MessageBody=message, MessageAttributes=attributes, MessageGroupId="1"
            )
            self._logger.info(
                {"title": "Sent Message", "message": message, "attributes": attributes}
            )
